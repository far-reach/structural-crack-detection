"""
Augment corrosion class to match crack/intact sample counts
...
SRC_TRAIN    = Path("data/codebrim/train/corrosion")
SRC_VAL      = Path("data/codebrim/val/corrosion")
TARGET_TRAIN = 900
TARGET_VAL   = 200
...
def augment_folder(src, target_count):
    ...
    while counter < needed:
        ...
        out     = src / f"aug_{counter:04d}_{src_img.stem}.jpg"
        aug.save(out, quality=90)
        counter += 1
    print(f"  Done: {len(list(src.glob('*')))} total images")

print("Augmenting train/corrosion...")
augment_folder(SRC_TRAIN, TARGET_TRAIN)

print("Augmenting val/corrosion...")
